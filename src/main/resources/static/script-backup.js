...
const APP = {
    ...
    renderCustomForm() {
        return `
          <form onsubmit="APP.handleCustom(event)">
              <div class="form-group">
                  <label>歌曲标题</label>
                  <input type="text" id="songTitle" placeholder="给你的歌曲起个名字" required>
              </div>
              <div class="form-group">
                  <label>自定义歌词</label>
                  <textarea id="customLyrics" placeholder="写下你的歌词..." required></textarea>
              </div>
              <div class="form-group">
                  <label>音乐风格标签</label>
                  <input type="text" id="styleTags" placeholder="例如: pop, rock, electronic">
              </div>
              <div class="form-group">
                  <label>版本选择</label>
                  <select id="mvVersion">
    ...
    async playMidiAudio() {
        ...
        if (this.audioContext.state === "suspended") {
            try {
                ...
                alert("Audio context needs to be resumed. Please try clicking play again.")
                return
            }
        }
